...
async function renderPosts() {
  ...
  // Banner News
  const mainBannerNews = generalNewsPosts[0];
  let mainBannerTemplate = `
    <a href="../pages/news/news-article-page.html?id=${mainBannerNews.id}">
  ...
  for (let i = 1; i < 5; i++) {
    ...
    <a href="../pages/news/news-article-page.html?id=${generalNewsPosts[i].id}">
      <div class="sub-news-control">
        <img src="../${generalNewsPosts[i].images.mainImg}" alt="${generalNewsPosts[i].title}" class="sub-news-img">
        <div class="sub-news-text-container">
          <h3 class="sub-news-heading">${generalNewsPosts[i].title}</h3>
        </div>
      </div>
    </a>
    `;
  ...
  // All News
  let template = " ";
  for (let i = 5; i < generalNewsPosts.length; i++) {
    template += `
