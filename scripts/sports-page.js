...
async function renderPosts() {
  ...
  // Banner News
  const mainBannerNews = sportsNewsPosts[0];
  let mainBannerTemplate = `
    <a href="../pages/sports/sports-article-page.html?id=${mainBannerNews.id}">
  ...
  for (let i = 1; i < 5; i++) {
    ...
    <a href="../pages/sports/sports-article-page.html?id=${sportsNewsPosts[i].id}">
      <div class="sub-news-control">
        <img src="../${sportsNewsPosts[i].images.mainImg}" alt="${sportsNewsPosts[i].title}" class="sub-news-img">
        <div class="sub-news-text-container">
          <h3 class="sub-news-heading">${sportsNewsPosts[i].title}</h3>
        </div>
      </div>
    </a>
    `;
  ...
  // All News
  let template = " ";
  for (let i = 5; i < sportsNewsPosts.length; i++) {
    template += `
        ...
        <a href="../pages/sports/sports-article-page.html?id=${sportsNewsPosts[i].id}">
          <div class="aside-article-control">
            <div class="aside-article-img-container">
              <img class="aside-article-img" src="../${sportsNewsPosts[i].images.mainImg}" alt="${sportsNewsPosts[i].title}"/>
            </div>
            <div class="aside-article-text-container">
              <h3 class="aside-article-heading ">${sportsNewsPosts[i].title}</h3>
